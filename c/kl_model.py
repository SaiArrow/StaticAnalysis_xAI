...
csv_file = 'attn_experiments.csv'
experiments = pd.read_csv(csv_file)
...
for i in X_tokens:
    for j in labels[i]:
        ...
        attn_tokens += [0] * int(MAX_LEN - len(attn_tokens))
...
class CustomTrainer(Trainer):
    ...
    def compute_loss(self, model, inputs, return_outputs=False,  num_items_in_batch=None, temperature=1.0):
        # Extract labels and convert them to long type for cross_entropy
        labels = inputs.pop("labels").long()
        rationale_mask = inputs["rationale_mask"]
        ...
        masked_input_ids[rationale_mask == 0] = mask_token_id

        outputs_masked = model(input_ids=masked_input_ids, attention_mask=attention_mask)
        logits_masked = outputs_masked.logits  # [batch, num_classes]

        log_probs_orig = F.log_softmax(logits / temperature, dim=-1)
        probs_masked = F.softmax(logits_masked / temperature, dim=-1)

        kl_loss = F.kl_div(log_probs_orig, probs_masked, reduction='batchmean') * (temperature ** 2)
...
lr = 5e-5
variations = [
["deepseek-ai/deepseek-coder-1.3b-base", lr],
["meta-llama/CodeLlama-7b-hf", lr],
["microsoft/wavecoder-ultra-6.7b", lr],
["google/codegemma-2b", lr],
["stabilityai/stable-code-3b", lr],
["Qwen/Qwen2.5-Coder-3B", lr],
["bigcode/starcoder2-3b", lr]
]
        ...
        for seed in [42, 1975, 5827, 7421, 9876]:
            ...
            for i, (train_index, test_index) in enumerate(sss.split(raw_text, label)):
                ...
                training_args = TrainingArguments(
                    output_dir = model_name+'_kl_sequence_classification_'+str(i),
                ...
                np.save("./saved_models/"+model_name+'_kl_sequence_classification_'+str(config)+str(seed)+str(i)+'test.npy',np.array(scores))
