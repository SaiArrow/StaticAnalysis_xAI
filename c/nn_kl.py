        ...
        for i, (train_index, test_index) in enumerate(sss.split(raw_text, label)):
            ...
            for epoch in range(num_epochs):
                ...
                for xb, yb, rationale_mask in train_loader:
                    ...
                    # Compute KL loss
                    masked_input = mask_input(xb, rationale_mask, mask_token_id=0)
                    output_masked = model(masked_input)

                    kl_loss = compute_kl_loss(output_orig, output_masked)
            ...
            np.save(config+str(seed)+str(i)+'egl.npy',tokens_gxi)
